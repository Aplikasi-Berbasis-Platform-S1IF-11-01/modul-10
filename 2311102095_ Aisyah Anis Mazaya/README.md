...
Dalam menjalankan aplikasi ini digunakan perintah `php -S localhost:8000` pada terminal untuk mengaktifkan built-in web server bawaan PHP. Pada perintah tersebut, `php` berfungsi untuk memanggil program Command Line Interface (CLI) dari PHP yang terinstal di sistem sedangkan parameter `-S` (kapital) menginstruksikan PHP untuk bertindak sebagai server web lokal alih-alih mengeksekusi skrip biasa. Selanjutnya `localhost` menentukan bahwa server ini hanya berjalan di lingkungan jaringan lokal komputer itu sendiri dan angka `8000` merupakan nomor port jaringan yang dialokasikan agar server dapat mendengarkan permintaan HTTP dari peramban. Penggunaan server lokal ini sangat krusial dalam implementasi AJAX karena jika file HTML dibuka secara langsung melalui File Explorer (menggunakan protokol `file:///`) peramban modern akan memblokir proses pengambilan data (fetch) ke file `data.php` akibat kebijakan keamanan CORS (Cross-Origin Resource Sharing). Dengan menjalankan perintah ini file disajikan melalui protokol `http://localhost:8000/` sehingga permintaan asinkron dapat diproses dengan aman dan lancar oleh peramban.
